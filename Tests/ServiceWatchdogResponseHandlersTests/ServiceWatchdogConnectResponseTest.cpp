...
TEST_CASE("Testing service-watchdog connect functionality", "") {
    ...
    Service::WatchdogConnectResponseHandler watchdogConnectResponseHandler{sequenceCode, watchdogConnectionState};

    SECTION("Parsing invalid message") {
        std::string invalidMessage{"abcd"}; // Just random string
        REQUIRE_THROWS_AS(watchdogConnectResponseHandler.handleResponse(invalidMessage), Service::WatchdogResponseHandlerException);
    }
    ...
    SECTION("Received success") {
        ...
        SECTION("Has sequence code") {
            ...
            watchdogConnectResponseHandler.handleResponse(connectMessage->body);
    ...
    SECTION("Received InvalidConnectionState") {
        ...
        connectResponseData.SerializeToString(&connectMessage->body);
        REQUIRE_THROWS_AS(watchdogConnectResponseHandler.handleResponse(connectMessage->body), Service::WatchdogResponseHandlerException);
        REQUIRE(watchdogConnectionState.getConnectionState() == Service::WatchdogConnectionState::ConnectionState::NotConnected);
    }
